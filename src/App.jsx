...
export default function App() {
  ...
  const carregarContadores = async () => {
    ...
    const { data, error } = await supabase
      .from('contabilistas')
      .select('*')
      .eq('aprovado', true) // só os aprovados aparecem no site
  ...
  const remover = async (id) => {
    const { error } = await supabase.from('contabilistas').delete().eq('id', id);
    if (!error) setContadores(prev => prev.filter(c => c.id !== id));
  };

  const aoSalvarCadastro = (novo) => {
    // não adiciona à lista porque precisa de aprovação primeiro
    setTela('sucesso');
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 max-w-sm w-full text-center">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-2">Perfil submetido!</h2>
          <p className="text-slate-500 text-sm mb-6">
            O teu perfil está a aguardar aprovação da equipa Me Contrata. Vais ser contactado pelo WhatsApp em breve.
          </p>
          <button
